...
export function GetModelInstance(data) {
    ...
    group("Model API: GetModelInstance", () => {
        ...
        check(http.request("POST", `https://${constant.grpcHost}/v1alpha/models/multipart`, fd_cls.body(), {
    ...
    group("Model API: LookUpModelInstance", () => {
        ...
        fd_cls.append("model_definition", model_def_name);
        fd_cls.append("content", http.file(constant.cls_model, "dummy-cls-model.zip"));
        let res_model = http.request("POST", `https://${constant.grpcHost}/v1alpha/models/multipart`, fd_cls.body(), {
            headers: genAuthHeader(
            data.userAccessToken,
            `multipart/form-data; boundary=${fd_cls.boundary}`
        ),
...
export function ListModelInstance(data) {
...
export function LookupModelInstance(data) {
    // LookUpModelInstance check
    group("Model API: LookUpModelInstance", () => {
        client.connect(constant.grpcHost, {});
